  ...
  for ((PSPLIT = $PSPLITMIN; PSPLIT <= $PSPLITMAX ; PSPLIT+=$PSPLITSTEP)); do
    ...
    # for ((QSPLIT = $PSPLIT; QSPLIT <= $PSPLIT ; QSPLIT+=$QSPLITSTEP)); do
      ...
      for ((I = 1; I <= $N; I++)); do
        ...
        # benchmark
        echo ppattern-benchmark --psize=$P --qsize=$Q --psplit=$PSPLIT --qsplit=$QSPLIT --seed=$SEED
        ../dist/build/ppattern-benchmark/ppattern-benchmark --psize=$P --qsize=$Q --psplit=$PSPLIT --qsplit=$QSPLIT --seed=$SEED >> ${CSV}
        echo
      done
    done
  done
